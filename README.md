...
- `**cpu-temp.sh**` — CPU temperature from sysfs `**/sys/class/hwmon/**` (`**k10temp**`, `**zenpower**`, or `**coretemp**`, `**temp1_input**` in millidegrees). Use when `**sensors**` is empty or not installed. Pass `**conky**` for `${color …}` tiers (same bands as GPU); in `**conky.text**` use `**${execpi … cpu-temp.sh conky}**` so Conky parses the markup.
...
- `**nvidia-conky.sh**` — GPU stats via `**nvidia-smi**`; subcommands `**gputemp**` / `**gputempconky**` (parsed colors, use `**${execpi …}**`) / `**gpuutil**` (and `**metrics**` for `temp + util` on one line); `**vram**` uses the same `**used/totalGb**` style for VRAM. Temp color tiers: **≥90** red, **≥80** orange, **≥70** yellow, **≥60** light yellow, else white.
